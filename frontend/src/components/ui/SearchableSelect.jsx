...
export function SearchableSelect({
  ...
  const dropdownPanel = isOpen ? (
    <div
      ...
      style={dropdownStyle}
      className="rounded-md bg-white border border-slate-200 shadow-dropdown max-h-64 overflow-hidden flex flex-col animate-in fade-in-50 duration-100"
    >
      <div className="p-2 border-b border-slate-100 bg-slate-50/80 shrink-0">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-1.5 text-xs bg-white rounded border border-brand-200 focus:outline-none focus:ring-1 focus:ring-brand-500 focus:border-brand-500"
          />
        </div>
      </div>

      <div className="overflow-y-auto max-h-48 overscroll-contain divide-y divide-slate-50">
            ...
            return (
              ...
              >
                <div className="min-w-0">
                  <div className="text-slate-800 truncate">{opt.label}</div>
                  {opt.subtext && (
                    <div className="text-[11px] text-slate-400 truncate">{opt.subtext}</div>
                  )}
                </div>
    ...
    <div className={cn('w-full relative', className)} ref={containerRef}>
      ...
      <div className="relative" ref={triggerRef}>
        ...
        >
          ...
          <ChevronDown
            className={cn(
              'h-4 w-4 text-slate-400 shrink-0 ml-2 transition-transform duration-150',
        ...
        {selectedOption && !disabled && (
          <button
            ...
            onClick={handleClear}
            className="absolute right-8 top-1/2 -translate-y-1/2 p-0.5 text-slate-400 hover:text-slate-600 rounded"
          >
